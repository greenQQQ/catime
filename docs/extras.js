...
(function () {
  "use strict";
  var OWNER = "greenQQQ/catime";
  var SISTER_OWNER = "greenQQQ/dogtime";
  var SISTER_NAME = "狗狗時光";
  var SISTER_URL = "https://greenqqq.github.io/dogtime/";
  ...
  function buildClockWall(ok, todays, today) {
    ...
    for (var h = 0; h < 24; h++) {
      ...
      if (cat) {
        ...
        cell.innerHTML = label + "<img loading='lazy' src='" + cat.url + "' alt='" + esc(cell.title) + "'>" + badge;
        (function (c2) {
          cell.addEventListener("click", function (e) {
            e.preventDefault();
            document.dispatchEvent(new CustomEvent("open-cat-lightbox", { detail: c2 }));
          });
        })(cat);
      } else {
    ...
    fetch(RAW + "cats/" + month + ".json")
      ...
      .then(function (details) {
        ...
        host.querySelector(".hl-img").addEventListener("click", function (e) {
          e.preventDefault();
          document.dispatchEvent(new CustomEvent("open-cat-lightbox", { detail: pick }));
        });
  ...
  function buildCrossover(ok) {
    ...
    fetch(SISTER_RAW + "catlist.json")
      ...
      .then(function (slist) {
        ...
        var half = function (cat, emoji, external) {
          if (!cat) return "<div class='xo-half xo-wait'><span>" + emoji + "</span><p>另一半還在畫…</p></div>";
          var img = "<img loading='lazy' src='" + cat.url + "' alt='" + esc(cat.title || "") + "'>";
          var cap = "<div class='xo-cap'>" + emoji + " #" + cat.number + (cat.title ? " " + esc(cat.title) : "") + "</div>";
        ...
        if (mineEl) mineEl.addEventListener("click", function () {
          document.dispatchEvent(new CustomEvent("open-cat-lightbox", { detail: mine }));
        });
        host.classList.remove("hidden");
      });
  }
